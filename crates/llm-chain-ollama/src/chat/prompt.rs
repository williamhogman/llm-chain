...
/// [`Role::System`], [`Role::User`] or [`Role::Assistant`] — Ollama takes system
/// instructions inline in the message list.
#[derive(Clone, Debug)]
...
/// A conversational template for Ollama's chat API: a list of
/// system/user/assistant message templates.
///
/// # Example
///
/// ```
/// use llm_chain_ollama::chat::{ChatPromptTemplate, Role};
///
...
/// ```
/// use llm_chain_ollama::chat::ChatPromptTemplate;
...
mod tests {
    ...
    fn formats_every_role() {
        ...
        assert_eq!(messages[0].content, "system x");
        assert_eq!(messages[2].role, Role::Assistant);
        assert_eq!(messages[2].content, "assistant x");
    }
    ...
    fn system_and_user_builds_two_messages() {
        ...
        assert_eq!(messages[1].content, "hello brief");
    }
}

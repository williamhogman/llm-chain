...
use super::types::{Content, Role};

/// A message prompt template consists of a role and a content. The role is either
/// [`Role::User`] or [`Role::Model`] — the Gemini API keeps system instructions
/// out of the conversation (see [`ChatPromptTemplate::with_system`]).
#[derive(Clone, Debug)]
...
impl MessagePromptTemplate {
    ...
    /// Formats the template into a concrete content entry.
    pub fn format(&self, parameters: &Parameters) -> Result<Content, FormatError> {
        Ok(Content::text(self.role, self.content.format(parameters)?))
    }
}
...
/// A conversational template for the Gemini API: optional system instructions
/// plus a list of user/model message templates.
///
/// # Example
///
/// ```
/// use llm_chain_gemini::generate_content::{ChatPromptTemplate, Role};
///
...
/// ```
/// use llm_chain_gemini::generate_content::ChatPromptTemplate;
...
impl ChatPromptTemplate {
    ...
    /// Formats every message template into concrete content entries.
    pub fn format(&self, parameters: &Parameters) -> Result<Vec<Content>, FormatError> {
        self.messages
            .iter()
